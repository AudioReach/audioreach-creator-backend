...
Data-link requests supply `linkType` from the following data-only contract:

```text
NORMAL | INTER_USECASE | EC
```
...
are in the same subgraph. Classification mismatches return `422` using the
existing validation format. Data-link API requests and responses do not expose
`isEc`.

Subsystem database rows do not persist classification. Resolved subsystem
links derive it from their canonical DataLink; pending unresolved subsystem
CREATE actions retain it in `edit_actions.newValue.linkType`.

The AWSP/UI metadata boundary remains unchanged: upload translates
`isEcLink` to `EC`, and download translates `EC` back to `isEcLink: true`.

The control-link contract is separate: `NORMAL | INTER_USECASE`, with the same
usecase-topology rules. Control-subsystem-link endpoint implementation is out
of scope here.
...
```
{
  ...
  linkType:                 NORMAL | INTER_USECASE | EC
}
```
...
### FR-DL-08 — Topology context is server-derived
...
### FR-DL-09 — linkType validation

The caller supplies `linkType`. The server loads the source and destination
subgraph and usecase context and validates:

- `NORMAL` requires the same usecase.
- `INTER_USECASE` requires different usecases.
- `EC` is invalid only when both endpoints are in the same subgraph.

The canonical DataLink persists the validated value.

### FR-DL-10 — No public EC flag

The request uses `linkType: EC`; it does not accept an `isEc` field.
...
2. Invoke `SubsystemBoundaryPathService` (see FR-SVC-01–03) to compute the traversal path and segment descriptors.
...
- `linkType`: as provided by the caller.
...
### FR-SVC-01 — Inputs and outputs (pure function contract)
...
### FR-SVC-02 — Pure function, no I/O
...
### FR-SVC-03 — Upload path refactored

`SubsystemBuilder.attachBoundaryPorts()` delegates to `SubsystemBoundaryPathService`. Upload behavior must not change — this is a pure code-movement refactor.
...
| D2 | EC classification is supplied as `linkType: EC`; automatic GKV-based EC detection is not added. | Keeps the API contract explicit; upload still translates existing `isEcLink` metadata. |
| D3 | If a link is POSTed where an active link already exists for the same `(sourcePort, destPort)` pair, the server rejects with `409`. | A port pair is a unique signal path; conflicting classification is a caller error, not a silent upgrade. |
...
| I7 | `NORMAL` requires source and destination endpoints in the same usecase. |
| I8 | `INTER_USECASE` requires source and destination endpoints in different usecases. |
| I9 | `EC` is invalid only when source and destination are in the same subgraph. |
| I10 | Data and control links use separate classification contracts; a control link cannot use `EC`. |
| I11 | Pending unresolved subsystem CREATEs retain `linkType` in `edit_actions.newValue`; subsystem tables do not own classification. |
...
| `linkType=NORMAL` but endpoints belong to different usecases | 422 |
| `linkType=INTER_USECASE` but endpoints belong to the same usecase | 422 |
| `linkType=EC` but endpoints belong to the same subgraph | 422 |
