...
Two separate endpoints, two separate command+handler pairs. Both share the same `DataLinkEditRepository` write port and `SubsystemDataLinkDerivationService` for traversal segment derivation.
...
Both handlers use:
  ...
  - SubsystemDataLinkDerivationService.compute()  (traversal segments)
  - DataLinkEditRepository                        (write port, new)
    └── PendingChangeWriter.writeCreate()         (edit_actions insertion)
```

`SubsystemDataLinkDerivationService` replaces `SubsystemBoundaryPathService`.
...
interface SubsystemRepository {
   ...
   *  Used by handlers to construct the nodeParentMap for SubsystemDataLinkDerivationService. */
...
8. **Segment derivation**: `SubsystemDataLinkDerivationService.compute({sourceNodeSystemId, destinationNodeSystemId, nodeParentMap})`
...
| FR-SVC-01–04 — derivation service replacement | §1, §5.1 steps 8-9 |
