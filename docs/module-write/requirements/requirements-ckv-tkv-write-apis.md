...
All new CKV/TKV entries must be seeded with default payloads derived from the module definition's `elementsStructure` JSON. The shared `serializeDefaultParameterData()` function implements this behavior.

### REQ-DEFAULT-01
`serializeDefaultParameterData(definition: ParameterDefinitionBase): SerializeResult` must be added to `packages/core/src/application/usecase-designer/shared/serialize-elements.ts` (shared location, reused by all new write APIs and referenced by existing TODOs).
...
Process request-validation failures independently. If one item references missing or duplicate data, continue processing the remaining items and return partial success. An unexpected persistence failure rolls back the request per REQ-XC-02.
...
Each new CKV is seeded with all parameters deduced per REQ-PARAMLIST-01/REQ-PARAMLIST-02. Each parameter payload is initialized using `serializeDefaultParameterData(param)`.
...
### REQ-ACK-06: Transaction atomicity
Each CKV's creation (CKV row + parameter payloads + zero-CKV removal) is part of the request's unit-of-work transaction. Any persistence failure rolls back all writes from that request.
...
Repeated value IDs within one item are normalized to a unique ordered list before comparison and persistence.
...
### REQ-RTG-04: Response
Return `TagInfoResponseDto[]` for all successfully removed tags, including the TKVs that were removed with each tag.
...
Process request-validation failures for each `CreateTkvRequestItem` independently. Unexpected persistence failures roll back the request.
...
Each new TKV is seeded with all parameters deduced per REQ-PARAMLIST-03 (TKV-scoped deduction). Each parameter payload is initialized using `serializeDefaultParameterData(param)`.
...
For each validated parameter, iterate all non-zero CKVs on the module. For any CKV that does not already have a payload for this parameter, stage CREATE of `CkvParameterPayload` seeded with `serializeDefaultParameterData(param)`.
...
For each (tkv, param) pair where the TKV does not already have a payload for that parameter, stage CREATE of `TkvParameterPayload` seeded with `serializeDefaultParameterData(param)`.
...
### REQ-XC-03: GroupId in command result
All write command results carry a `groupId` internally for undo/redo orchestration. REST response DTOs continue to use the project-wide `ApiResult` contract and do not expose `groupId`.
...
### REQ-XC-06: Module parameter tool policy
The `ModuleParameterDefinition` interface returned by `getParameterDefinitions()` must include `toolPolicy: string` so handlers can filter to CALIBRATION-capable params without widening the serializer's shared `ParameterDefinitionBase` contract.
...
### REQ-XC-08: Partial-success status
Any endpoint that returns `Result.partial()` must document and emit HTTP 207 with the same response data schema as HTTP 200 plus the standard `issues` array.
