...
The serializer and parser use the same layout rules: apply each element's explicit alignment before processing it, pad each completed struct to a four-byte boundary, and pad the complete parameter payload to an eight-byte boundary. Formula-driven arrays use the evaluated positive length and otherwise fall back to `arrayLength`.
...
function buildDefaultElements(schema: DefinitionElement[]): ElementData[] {
...
### 2.2 `ModuleParameterDefinition` — add `toolPolicy` field

**File:** `packages/core/src/application/ports/persistence/repositories/module/module-definition.repository.ts`

Add `toolPolicy: string` to `ModuleParameterDefinition`, leaving the shared serialization shape unchanged:
...
export interface ModuleParameterDefinition extends ParameterDefinitionBase {
  isReadOnly: boolean;
  toolPolicy: string;  // CALIBRATION when any stored policy entry supports it
}
```
...
**Note:** `toolPolicy` is stored as a JSON array in the DB column `tool_policies`. Return `CALIBRATION` when any entry supports calibration; otherwise return the first policy.
...
**Updated `CkvOverlayFetcher.fetchMany()` and `fetchOne()`:** Both methods check CREATE actions for Ckv. When they encounter a CREATE action, they read `valueDefinitionSystemIds` from the action's `newValue` payload and populate `OverlaidCkv.values` from it instead of from the base table.
...
getAllTkvsForTag(tagSystemId: number, spfModuleSystemId: number): Promise<TkvSummary[]>;

// Returns a single Tkv row by systemId
getTkvBySystemId(tkvSystemId: number, spfModuleSystemId: number): Promise<TkvSummary | null>;
...
// Stages DELETE for Tkv + all children; TKV actions use the owning tag as aggregateId
removeTkv(tkvSystemId: number, tagSystemId: number, options?: EditOptions): Promise<void>;
...
addParameterToCkv(ckvSystemId: number, moduleSystemId: number, parameterSystemId: number, payloadSystemId: number, payload: Uint8Array, options?: EditOptions): Promise<void>;
...
addParameterToTkv(tkvSystemId: number, tagSystemId: number, parameterSystemId: number, payloadSystemId: number, payload: Uint8Array, options?: EditOptions): Promise<void>;

// Stages DELETE for TkvParameterPayload
removeParameterFromTkv(payloadSystemId: number, tkvSystemId: number, tagSystemId: number, options?: EditOptions): Promise<void>;
```
...
4. Fetch `ModuleParameterDefinition[]` for those param systemIds.
5. For each `CreateCkvRequestItem`:
   a. Resolve all value definitions; collect an item issue and skip if any are missing.
   b. Check duplicate (same `valueDefinitionSystemIds` exists → skip).
   c. Allocate `ckvSystemId = idGeneration.getNextId(fileSystemId)`.
   d. For each param: call `serializeDefaultParameterData(def)` to get `Uint8Array`.
   e. Build `KvData` with all parameter payloads.
   f. `repo.createCkv(kvData, moduleSystemId)`.
   g. If this was the first non-zero CKV AND a zero CKV existed → `repo.removeCkv(zeroCkvSystemId, moduleSystemId)`.
6. `uow.commit()`.
7. Return the internal command result `{groupId, addedCkvs, removedCkvSystemIds}`; the controller maps only the DTO fields to the REST response.

**Error handling:** Per-item validation failures are collected in `Result.partial()`. Unexpected write failures roll back the request transaction.
...
3. For each `ckvSystemId`:
   ...
   b. `repo.removeCkv(ckvSystemId, moduleSystemId)`.
...
**Note:** The existing `TagDefinitionQueryService.getTagDefinition()` supplies `tagId` and `tagName` for validation and response mapping.
...
3. `uow.commit()`.
4. Return removed `TagInfoResponseDto[]`, including the fully populated TKVs removed with each tag.
...
5. Fetch `ModuleParameterDefinition[]`.
6. For each `CreateTkvRequestItem`:
   a. Resolve all value definitions; collect an item issue and skip if any are missing.
   b. Duplicate check.
   c. Allocate `tkvSystemId`.
   d. Build `KvData` with default payloads.
   e. `repo.createTkv(kvData, tagSystemId, moduleSystemId)`.
7. `uow.commit()`.
8. Return `TkvDto[]`.
...
2. For each `tkvSystemId`:
   ...
   b. `repo.removeTkv(tkvSystemId, tagSystemId)`.
3. `uow.commit()`.
...
async addCkvs(...): Promise<ApiResult<AddCkvsResponseDto>> {
  ...
  return toApiResult(result, data => ({addedCkvs: ..., removedCkvSystemIds: ...}));
}
```
...
Controller.addCkvs(projectId, spfModuleSystemId, CreateCkvsRequestDto)
  → new AddCkvsCommand(spfModuleSystemId, ckvs[])
  ...
  → toApiResult() → AddCkvsResponseDto
```
...
`AddTagsHandler` uses the existing `TagDefinitionQueryService.getTagDefinition()` lookup to validate each definition and populate `tagId` and `tagName` in `TagInfoResponseDto`. No write-side repository method is required.
...
The global partial-success interceptor converts `Result.partial` API envelopes to HTTP 207; controllers use the existing `toApiResult()` mapper.
Each endpoint capable of producing a partial result documents that 207 response with the same data DTO as its 200 response; the standard `ApiResult.issues` field carries per-item failures.
...
2. **Integration tests** (`packages/infrastructure/persistence/tests/integration/`) — staged CKV/TKV values, aggregate ownership, and explicit payload-delete cascades are tested against SQLite.
