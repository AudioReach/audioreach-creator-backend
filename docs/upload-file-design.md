...
sequenceDiagram
    ...
    Client->>Handler: handle(UploadFileCommand)
    ...
    Handler-->>Client: UploadFileResult{projectId, errors, warnings}
```
...
**Key Methods**:
- `handle(command: UploadFileCommand): Promise<UploadFileResult>`
...
3. Dispatch UploadFileCommand → UploadFileHandler
