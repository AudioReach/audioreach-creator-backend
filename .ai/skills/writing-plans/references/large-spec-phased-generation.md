...
- Chapter partition: names, LLD section references, batch grouping, task start numbers
...
- **Project context source.** The CLAUDE.md path. The subagent reads it itself to learn package names and test commands.
- **Output format reference.** The absolute path to `plan-format.md`. Tell the subagent: "Follow this file's Task Structure, Bite-Sized Task Granularity, Code Completeness by Task Type, Skeleton Format for complex handlers, and No Placeholders rules. The format file is the single source of truth — do not paraphrase from memory."
...
**Typical dependency order for a CQRS/hexagonal backend feature:**

| Batch | Chapters | Can be parallel |
|---|---|---|
| 1 | Entities & enums, Persistence schemas, Domain services | Yes |
| 2 | Port interfaces, Commit orchestration | Yes (after batch 1) |
| 3 | Command handlers | No — needs port types from batch 2 |
| 4 | Unit tests, Integration tests, E2E tests | Yes (after batch 3) |
