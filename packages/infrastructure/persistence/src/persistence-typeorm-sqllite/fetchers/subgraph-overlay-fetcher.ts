 ...
 *
 * Works on the scalar-column base types (SubgraphBase / SgkvBase) — mirrors
 * ModuleNodeOverlayFetcher which uses SpfModuleBase. Relation fields (sgkvs,
 * values, file, …) are not the fetcher's concern; callers preserve and
 * re-merge those from the original loaded rows after the overlay is applied.
 *
