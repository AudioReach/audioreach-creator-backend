...
/**
 * Full overlaid scalar row from control_links — all fields needed by
 * ControlLinkReadModel. Returned by the subgraph/usecase-scoped methods;
 * distinct from LinkOverlayEntry which carries only linkSystemId+portSystemId
 * for port-level link counting.
 */
export interface OverlaidControlLink {
  systemId: number;
  fileSystemId: number;
...
/**
 * Full overlaid scalar row from data_links — all fields needed by
 * DataLinkReadModel.
 */
export interface OverlaidDataLink {
  systemId: number;
  fileSystemId: number;
 ...
 * fetchControlLinksBySubgraphId) that return full OverlaidControlLink rows for
...
export class LinkOverlayFetcher {
  ...
  async fetchDataLinksBySubgraphId(
    ...
    const baseRows = (await this.manager
      ...
      .getMany()) as unknown as OverlaidDataLink[];
  ...
  private async applyDataLinkOverlay(
    ...
    if (sessionId === null) {
      ...
      const created: OverlaidDataLink[] = actions
        .filter(
          (a: EditActionRow) =>
        ...
        .map((a: EditActionRow) => {
          const p = a.newValue as Partial<OverlaidDataLink>;
          return {
  ...
  async fetchSubsystemControlLinkSegmentsByUsecaseIds(
    ...
    fileSystemId: number,
    sessionId: number | null,
  ): Promise<OverlaidControlLink[]> {
    if (usecaseSystemIds.length === 0) return [];

    const baseRows: OverlaidControlLink[] = await this.manager
      .createQueryBuilder()
      .distinct(true)
      .select([
  ...
  async fetchSubsystemDataLinkSegmentsByUsecaseIds(
    ...
    const baseRows: OverlaidDataLink[] = await this.manager
      .createQueryBuilder()
      .distinct(true)
      .select([
  ...
  private async applySubsystemLinkOverlay(
    ...
    const baseIds = new Set(baseRows.map(r => r.systemId));
    const created: OverlaidControlLink[] = actions
      .filter(
        (a: EditActionRow) =>
          a.operation === CHANGE_OPERATION.Create &&
          !baseIds.has(a.targetSystemId),
      ...
      .map((a: EditActionRow) => {
        const p = a.newValue as Partial<OverlaidControlLink>;
        return {
          systemId: a.targetSystemId,
          fileSystemId: p.fileSystemId ?? fileSystemId,
  ...
  async fetchControlLinksBySubgraphId(
    ...
    const baseRows = (await this.manager
      ...
      .getMany()) as unknown as OverlaidControlLink[];
  ...
  async fetchControlLinksByUsecaseIds(
    ...
    const merged = [
      ...intraSubgraph,
      ...intraUsecase,
    ] as unknown as OverlaidControlLink[];
  ...
  private async applyControlLinkOverlay(
    ...
    let allRows: OverlaidControlLink[];
