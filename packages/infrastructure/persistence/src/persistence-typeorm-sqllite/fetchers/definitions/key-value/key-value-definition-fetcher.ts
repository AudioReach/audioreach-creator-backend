...
export interface OverlaidValueDefinition {
  systemId: number;
  /** FK to the parent key definition (arc_keys.system_id). */
  keySystemId: number;
  valueId: number;
  name: string;
  description: string | undefined;
  enumMember: string | undefined;
  specialValue: string | undefined;
}

/**
 * Overlaid scalar fields from arc_keys, with values nested.
 * Used by toKeyDefinitionReadModel (key-definition-row-mapper.ts) and the
 * service's value-first resolution path.
 */
export interface OverlaidKeyDefinition {
  systemId: number;
  fileSystemId: number;
  keyId: number;
  name: string;
  description: string | undefined;
  isCalibrationKey: boolean | undefined;
  isGraphKey: boolean | undefined;
  isVoice: boolean | undefined;
  isDynamic: boolean | undefined;
  specialityKeyValue: string | undefined;
  enumMember: string | undefined;
  enumName: string | undefined;
  calKeyEnumMember: string | undefined;
  graphKeyEnumMember: string | undefined;
  /** Overlay-aware child values for this key. */
  values: OverlaidValueDefinition[];
}

interface KeyDefinitionBase {
  systemId: number;
  fileSystemId: number;
  keyId: number;
  name: string;
...
export class KeyValueDefinitionFetcher {
  ...
  async fetchByKeySystemIds(
    ...
    const valuesByKeyId = new Map<number, OverlaidValueDefinition[]>();
    for (const v of allValues) {
      const bucket = valuesByKeyId.get(v.keySystemId) ?? [];
      bucket.push(this.toOverlaidValue(v));
  ...
  async fetchValuesBySystemIds(
    ...
    sessionId: number | null,
  ): Promise<OverlaidValueDefinition[]> {
    ...
    if (sessionId === null) return baseRows.map(r => this.toOverlaidValue(r));
    ...
    ).map(r => this.toOverlaidValue(r.effective));

    const baseIds = new Set(baseRows.map(r => r.systemId));
    const created: OverlaidValueDefinition[] = relevantActions
      .filter(
        (a: EditActionRow) =>
          a.operation === CHANGE_OPERATION.Create &&
  ...
  private toOverlaidKey(
    r: KeyDefinitionBase,
  ): Omit<OverlaidKeyDefinition, 'values'> {
    return {
      systemId: r.systemId,
      ...
      isCalibrationKey: r.isCalibrationKey,
      isGraphKey: r.isGraphKey,
      isVoice: r.isVoice,
      isDynamic: r.isDynamic,
      specialityKeyValue: r.specialityKeyValue,
      enumMember: r.enumMember,
      enumName: r.enumName,
      calKeyEnumMember: r.calKeyEnumMember,
      graphKeyEnumMember: r.graphKeyEnumMember,
    };
  }

  private toOverlaidValue(r: ValueDefinitionBase): OverlaidValueDefinition {
    return {
      systemId: r.systemId,
      keySystemId: r.keySystemId,
      valueId: r.valueId,
      name: r.name,
      description: r.description,
      enumMember: r.enumMember,
