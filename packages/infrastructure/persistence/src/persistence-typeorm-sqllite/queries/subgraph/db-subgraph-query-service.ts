...
export class DbSubgraphQueryService implements SubgraphQueryService {
  ...
  async findPropertyPayloads(
    ...
    try {
      // Step 1 — load base subgraph scalar row (existence check)
      const baseSubgraph = (await this.dataSource
        ...
        .where('s.systemId = :id AND s.fileSystemId = :file', {
          id: subgraphSystemId,
          file: fileSystemId,
        })
        .getOne()) as SubgraphBase | null;

      // Step 2 — load base property rows
      const basePropRows = (await this.dataSource
        ...
        .where('spd.subgraphSystemId = :id', {id: subgraphSystemId})
        .getMany()) as SubgraphPropertyDataBase[];

      // Step 3 — no session: return base data directly
      const session =
        await this.sessionRepo.findActiveSessionByFileSystemId(fileSystemId);
      if (!session) {
        if (!baseSubgraph) return Result.ok(null);
        return Result.ok(basePropRows.map(p => this.toPropertyPayload(p)));
      }

      // Step 4 — session: overlay subgraph existence (CREATE/DELETE handling)
      const overlaidSubgraphs = await this.subgraphFetcher.applyToSubgraphs(
        baseSubgraph ? [baseSubgraph] : [],
        fileSystemId,
        session.sessionId,
      );
      if (overlaidSubgraphs.length === 0) return Result.ok(null);

      // Step 5 — overlay property rows
      const overlaidProps = await this.subgraphFetcher.applyToPropertyRows(
        basePropRows,
        subgraphSystemId,
        session.sessionId,
      );
      return Result.ok(overlaidProps.map(p => this.toPropertyPayload(p)));
    } catch (error) {
      return Result.fail({
