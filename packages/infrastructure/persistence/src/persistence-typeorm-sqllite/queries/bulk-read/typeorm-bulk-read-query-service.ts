...
export class TypeOrmBulkReadQueryService implements BulkReadQueryService {
  ...
  private buildCalibrationModels(
    ...
    for (const row of paramRows) {
      ...
      paramMap.get(row.ckvSystemId)!.push({
        naturalId: row.spfParameter!.naturalId,
        payload: row.payload!,
    ...
    for (const ckv of sortedEntries) {
      ...
      currentKvCombo.modules.push({
        naturalId: ckv.module!.naturalId,
  ...
  private sortVcpmCkvEntries(entries: VcpmCkvRow[]): VcpmCkvRow[] {
    ...
    prepared.sort((a, b) => {
      const sgA = a.entry.vcpmInstance?.subgraph?.naturalId ?? 0;
      const sgB = b.entry.vcpmInstance?.subgraph?.naturalId ?? 0;
      if (sgA !== sgB) return sgA - sgB;
  ...
  private buildVcpmCalibrationModels(
    ...
    for (const ckv of sortedEntries as Augmented[]) {
      const subgraphId = ckv.vcpmInstance.subgraph.naturalId;
      const vals = [...(ckv.values ?? [])].sort(
        (x, y) =>
          (x.valueDef?.keys?.naturalId ?? 0) -
          (y.valueDef?.keys?.naturalId ?? 0),
    ...
    for (const sg of result) {
      ...
      sg.masterKeys = [...mkMap.entries()]
        .sort(([a], [b]) => a - b)
        .map(([keyNaturalId, isDynamic]) => ({keyNaturalId, isDynamic}));
    }
  ...
  private buildTagDataModels(
    ...
    const paramMap = new Map<
      number,
      Array<{naturalId: number; payload: Uint8Array}>
    >();
    for (const row of paramRows) {
    ...
    for (const row of rows) {
      ...
      for (const tkv of row.tkvs ?? []) {
        ...
        current.tkvs.push({
          ...
          modules: [
            {
              naturalId: row.module!.naturalId,
    ...
    for (const row of paramRows) {
      ...
      if (row.payload) {
        paramMap.get(row.dkvSystemId)!.push({
          naturalId: row.driverParameter!.naturalId,
          payload: row.payload,
        });
      }
    }
