...
export class ControlPortDto extends BaseComponentDto<number> {
    ...
    @ApiProperty({ description: 'Control port intents', type: [ControlPortIntentDto] })
    ...
    get componentType(): ComponentInfoType {
        return ComponentInfoType.ControlPort;
    }

    constructor(systemId: string, id: number);
    constructor(systemId: string, id: number, name: string, portType: PortType, intents: ControlPortIntentDto[], isVirtual?: boolean);
    constructor(systemId: string, id: number, name?: string, portType?: PortType, intents?: ControlPortIntentDto[]) {
        super(systemId, id);
