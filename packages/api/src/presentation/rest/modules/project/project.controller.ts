import { Body, Controller, Delete, Get, HttpStatus, Param, Patch, Post, UploadedFiles, UseGuards, UseInterceptors } from '@nestjs/common';
import { ApiBody, ApiConsumes, ApiExtraModels, ApiOperation, ApiParam, ApiResponse, getSchemaPath } from '@nestjs/swagger';
import { ProjectDetails, ProjectType, SessionMode } from './dtos/project-details-response.dto.js';
...
export class ProjectController {
  ...
  @ApiOperation({
    ...
    description: 'Creating a new project while opening acdb and workspace files',
  })
  ...
  async uploadArcDbFiles(
    ...
    const projectdetails: ProjectDetails = { projectId: '', projectName: '', projectDescription: ' ', projectType: ProjectType.Offline, sessionMode: SessionMode.Designer }; // ToDo Need to update the project Info once services ready
  ...
  async getProject(@Param('projectId') _projectId: string): Promise<ApiResult<ProjectDetails>> {
 ...
 @ApiParam({ name: 'projectId', description: 'Id of project', required: true })
  ...
  async connectToProject(@Param('projectId') _projectId: string): Promise<ApiResult<ProjectDetails>> {
  ...
  async disconnectFromProject(@Param('projectId') _projectId: string): Promise<ApiResult<ProjectDetails>> {
    // Need a project Id to open the project. It will take from header
  ...
  async downloadArcDbFiles(@Param('projectId') _projectId: string): Promise<ApiResult<DownloadArcDatabaseFilesResponse>> {

    await Promise.resolve();
    const acdbWorkspaceFilesResponse: DownloadArcDatabaseFilesResponse = new DownloadArcDatabaseFilesResponse;
  ...
  @ApiOperation({ summary: 'Delete project', description: 'Deleting the project based on project Id.' })
  @ApiParam({ name: 'projectId', description: 'Id of project', required: true })
  @ApiResponse({
  ...
  async deleteProject(@Param('projectId') _projectId: string): Promise<ApiResult<null>> {
  ...
  @ApiParam({ name: 'systemId', description: 'Id provided by system', required: true })
  @ApiOperation({
  ...
  async getDeviceDetails(@Param('systemId') _systemId: string): Promise<ApiResult<DeviceDetailInfo>> {
  ...
  @ApiBody({
    description: 'Upload the WorkspaceFile if it is not already available on the device.',
    schema: {
      type: 'object',
      properties: {
  ...
  async connectToDevice(@Param('systemId') _systemId: string, @UploadedFiles() _workspaceFile?: Multer, @Body() _connectToDeviceRequestDto?: ConnectToDeviceRequest): Promise<ApiResult<ProjectDetails>> {

    await Promise.resolve();
    const projectResponse: ApiResult<ProjectDetails> = new ApiResult<ProjectDetails>
    return projectResponse;
  }
}
