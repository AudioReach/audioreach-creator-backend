...
export class ContainerController extends BaseController {
    ...
    */
    ...
    async getContainers(
        ...
        console.log(`Getting containers in project ${projectId}: ${JSON.stringify(request)}`);
    ...
    @ApiParam({ name: 'containerSystemId', required: true, type: String, description: 'System id of a container' })
    @ApiDocumentationWithExample({
    ...
    async getContainerProperties(
        ...
        await Promise.resolve(); // Placeholder to satisfy linter
        console.log(`Getting properties in project ${projectId} for container ${containerSystemId}`);
        throw new HttpException(
