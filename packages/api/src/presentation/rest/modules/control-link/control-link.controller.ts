import { Controller, Post, Get, Patch, Body, Param, UseGuards, HttpStatus, HttpException } from '@nestjs/common';
...
import { ControlLinkDto, ControlLinkPropertiesDto } from './dto/control-link.dto.js';
...
export class ControlLinkController extends BaseController {
    ...
    async getControlLinks(
        ...
        console.log(`Getting control-links in project ${projectId}: ${JSON.stringify(controlLinkSystemIds)}`);
    ...
    async addControlConnection(@Body() controlLinkRequest: NewLinkRequest): Promise<ApiResult<ControlLinkDto>> {
    ...
    async updateControlLinkProperties(
        ...
        console.log(`Updating control link ${controlLinkSystemId} with properties:`, properties);
        await Promise.resolve(); // Placeholder to satisfy linter
        throw new HttpException(
    ...
    @ApiParam({ name: 'controlLinkSystemId', required: true, type: String, description: 'System id of a control link' })
    @ApiDocumentationWithExample({
    ...
    async getControlLinkProperties(
        ...
        await Promise.resolve(); // Placeholder to satisfy linter
        console.log(`Getting properties in project ${projectId} for control link ${controlLinkSystemId}`);
        throw new HttpException(
