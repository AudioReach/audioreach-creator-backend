import { Controller, Get, Post, UseGuards, HttpStatus, HttpException, Param, Body, Query } from '@nestjs/common';
...
@ApiExtraModels(UsecaseIdentifier,
    UsecaseDto,
    UsecaseWithModificationSummary,
    BaseComponentDto,
    ModuleInstanceDto)
export class UseCaseController extends BaseController {
    ...
    @ApiDocumentationWithExample({
        ...
        description: "Returns array of usecases in a unified format that handles two scenarios:\n\n" +
        ...
        responses: [
            {
                ...
                description: 'Use cases are returned successfully with scenario indicators',
                dto: [UsecaseDto],
                example: {
    ...
    async getAllUsecases(@Param('projectId') projectId: string): Promise<ApiResult<UsecaseDto[]>> {
    ...
    async getUsecasesForSubgraph(
        ...
        @Query('subgraphSystemId') subgraphSystemId: string): Promise<ApiResult<UsecaseIdentifier[]>> {
        await Promise.resolve(); // Placeholder to satisfy linter
        console.log(`Getting all usecases for project: ${projectId} and subgraph: ${subgraphSystemId}`);
        throw new HttpException(
    ...
    @Post('components/get')
    @ApiDocumentationWithExample({
        summary: 'Get all components (including module instances, data links, control links, subsystems) for usecase(s) based on querying type.',
        description: "For components shared in different usecases, only one copy will be returned.",
    ...
    @ApiQuery({
        name: 'contentsType', required: false, enum: ComponentsTypeInUsecase,
        description: 'Type of contents to be returned. Optional. If not provided, default is TopLevelComponents.\n\n' +
    ...
    async getComponentsInUsecases(
        ...
        @Query('contentsType') contentsType: ComponentsTypeInUsecase = ComponentsTypeInUsecase.TopLevel,
    ): Promise<ApiResult<UsecaseComponentsDto[]>> {
        await Promise.resolve(); // Placeholder to satisfy linter
        console.log(`Getting components for usecases in project ${projectId}: ${JSON.stringify(usecaseSystemIds)}, contentsType: ${contentsType}`);
        throw new HttpException(
    ...
    */
    @Get('updates/summary')
    @ApiDocumentationWithExample({
        summary: 'Get all added and deleted usecases with their modification summary',
        responses: [
            {
                status: HttpStatus.OK,
                description: 'Success',
    ...
    async getUsecaseModificationSummary(@Param('projectId') projectId: string): Promise<ApiResult<UsecaseWithModificationSummary[]>> {
        await Promise.resolve(); // Placeholder to satisfy linter
    ...
    async deleteUsecases(
        ...
        await Promise.resolve(); // Placeholder to satisfy linter
        console.log(`Deleting usecases for project ${projectId}: ${JSON.stringify(usecaseSystemIds)}`);
        throw new HttpException(
