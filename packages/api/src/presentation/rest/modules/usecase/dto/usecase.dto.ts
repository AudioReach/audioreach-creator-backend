...
import { IsBoolean, IsArray, ValidateIf, ArrayMinSize, ArrayMaxSize, IsOptional } from 'class-validator';
...
export enum UsecaseType {
    SubsystemFiltered = 'SubsystemFiltered',
    Ec = 'Ec',
    Regular = 'Regular',
    Manual = 'Manual'
}
...
export class UsecaseIdentifier extends KVInfo {
    ...
    constructor(systemId: string, useCaseType: UsecaseType, kvInfo: KVInfo, aliasId?: number, aliasName?: string, category?: string) {
...
export class UsecaseDto {
    ...
    private _filteredKV: KVInfo | undefined;
    private _isFiltered: boolean;

    @ApiProperty({
        description: 'Indicates whether this usecase has subsystem filtering applied. ' +
    ...
    @IsArray()
    @ValidateIf((o: UsecaseDto) => !o.isFiltered)
    @ArrayMinSize(1)
    @ArrayMaxSize(1)
    @ValidateIf((o: UsecaseDto) => o.isFiltered)
    @ArrayMinSize(1)
    ...
    @ApiProperty({
        description: 'Subsystem filtered key-value information. ' +
            'undefined when isFiltered=false (No FilteredKV scenario), ' +
            'populated when isFiltered=true (subsystem filtered scenario)',
        type: KVInfo,
        nullable: true,
        required: false
    })
    @ValidateIf((o: UsecaseDto) => !o.isFiltered, { message: 'filteredKV must be undefined when isFiltered is false' })
    @IsOptional()
    @ValidateIf((o: UsecaseDto) => o.isFiltered, { message: 'filteredKV is required when isFiltered is true' })
    get filteredKV(): KVInfo | undefined {
        return this._filteredKV ?? undefined;
    }
     ...
     */
    constructor(systemId: string, filteredKv: KVInfo, usecases: UsecaseIdentifier[]);
    ...
    constructor(
        ...
        if (typeof rawUsecaseOrSystemId === 'string') {
            ...
            if (!filteredKv || !usecases) {
                throw new Error('filteredKv and usecases are required for filtered scenario');
            }
            this._isFiltered = true;
            this._filteredKV = filteredKv;
            this._filteredKV.systemId = rawUsecaseOrSystemId;
            this._usecases = usecases;
        } else {
            // Raw GKV scenario (Scenario 1)
            this._isFiltered = false;
            this._filteredKV = undefined;
    ...
    validate(): { isValid: boolean; errors: string[] } {
        ...
        if (this._isFiltered === false) {
            // Raw GKV scenario validation
            if (this._filteredKV !== undefined) {
                errors.push('filteredKV must be undefined when isFiltered is false');
            }
            if (this._usecases.length !== 1) {
                errors.push('usecases array must contain exactly one item when isFiltered is false');
            }
        } else if (this._isFiltered === true) {
            // Filtered GKV scenario validation
            if (this._filteredKV === undefined) {
                errors.push('filteredKV cannot be undefined when isFiltered is true');
            }
            if (this._usecases.length === 0) {
                errors.push('usecases array must contain at least one item when isFiltered is true');
            }
        }
    ...
    assertValid(): void {
        ...
        if (!validation.isValid) {
            throw new Error(`SubsystemFilteredUsecase validation failed: ${validation.errors.join(', ')}`);
        }
    }
}
...
export class UsecaseWithModificationSummary {
    ...
    constructor(usecaseWithComponents: UsecaseWithComponents,
