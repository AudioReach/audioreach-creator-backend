import { Controller, Body, Param, HttpStatus, HttpException, UseGuards, Post } from '@nestjs/common';
...
export class SubsystemController extends BaseController {
    ...
    async getSubsystems(
        ...
        console.log(`Getting subsystems in project ${projectId}: ${JSON.stringify(subsystemSystemIds)}`);
    ...
    @ApiDocumentationWithExample({
        summary: 'Get all top level components in a subsystem for provided usecases',
        requestDto: SystemIdsRequestDto,
        requestDtoDescription: "List of system ids for usecases. Optional.\n\n" +
    ...
    async getComponentsInSubsystem(
        ...
        await Promise.resolve(); // Placeholder to satisfy linter
        console.log(`Getting components for subgraph ${subsystemSystemId} in project ${projectId} with optional usecase system ids: ${JSON.stringify(usecaseSystemIds)}`);
        throw new HttpException(
