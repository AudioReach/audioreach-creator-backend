...
import { ApiOperation, ApiResponse, ApiBody, getSchemaPath, ApiBodyOptions, ApiResponseOptions } from '@nestjs/swagger';
 ...
 */
function isArrayDto(dto: Type<unknown> | Type<unknown>[]): dto is Type<unknown>[] {
 ...
 */
function shouldTreatAsArray(dto: Type<unknown> | Type<unknown>[] | undefined, explicitFlag?: boolean): boolean {
    if (!dto) return false;
...
function createRequestBodyConfig(options: ApiDocumentationOptions) {
    const isArray = shouldTreatAsArray(options.requestDto, options.isRequestArray);
    const baseType = options.requestDto ? getBaseType(options.requestDto) : undefined;
 ...
 */
function addResponseExampleConfig(config: Record<string, unknown>, options: ApiDocumentationOptions) {
...
function loadExampleDynamicallySync(exampleConfig: ExampleConfig): unknown {
    try {
        ...
        if (!modulePath.endsWith('.js') && !modulePath.endsWith('.ts')) {
            ...
            modulePath = existsSync(`${modulePath}.js`) ? `${modulePath}.js` : `${modulePath}.ts`;
        }

        const moduleContent = loadModule(modulePath);
        const ExampleClass = getExampleClass(moduleContent, exampleConfig.className);

        if (!ExampleClass) {
            console.warn(`Class ${exampleConfig.className} not found in module ${exampleConfig.modulePath}`);
            return getPlaceholderExample(exampleConfig.className);
        }

        return createExampleInstance(ExampleClass);
    } catch (error: unknown) {
        console.error(`Error loading example from ${exampleConfig.modulePath}:`, error);
        return getPlaceholderExample(exampleConfig.className);
    }
}
...
function loadModule(modulePath: string): Record<string, unknown> {
    try {
        ...
        console.warn(`Could not load module ${modulePath}. Using placeholder example.`);
        return {};
    }
}
...
function getExampleClass(moduleContent: Record<string, unknown>, className: string): ExampleClass | undefined {
    ...
    if (moduleContent.default && typeof moduleContent.default === 'object') {
        console.log('Default export keys:', Object.keys(moduleContent.default as Record<string, unknown>));
    }
