...
describe('GetComponentsWithSubsystemsHandler', () => {
  ...
  describe('Scenario H3 — initial state SS(m3,m4): SLS segments fetched and folded into dataLinks', () => {
    ...
    it('places the outside SLS segment SLS_L2_OUT (m2→SS) at root dataLinks', () => {
      if (result.kind !== RESULT_KIND.Ok) return;
    ...
    it('places the inside SLS segment SLS_L3_IN (SS→m3) at SS dataLinks', () => {
      if (result.kind !== RESULT_KIND.Ok) return;
  ...
  describe('Scenario H4 — edit session adds SS1: new subsystem appears in tree', () => {
    ...
    it('places the outside SLS segment SLS_L5_OUT (m4→SS1) at SS dataLinks', () => {
      if (result.kind !== RESULT_KIND.Ok) return;
      const ss = result.data.subsystems.find(s => Number(s.systemId) === SS)!;
      expect(collectDataLinkIds(ss.children)).toContain(SLS_L5_OUT.systemId);
    });
    ...
    it('places the inside SLS segment SLS_L6_IN (SS1→m5) at SS1 dataLinks', () => {
      if (result.kind !== RESULT_KIND.Ok) return;
      const ss = result.data.subsystems.find(s => Number(s.systemId) === SS)!;
      const ss1 = ss.children.subsystems.find(s => Number(s.systemId) === SS1)!;
      expect(collectDataLinkIds(ss1.children)).toContain(SLS_L6_IN.systemId);
    });
  ...
  describe('Scenario H5 — edit session deletes SS1: tree reverts to initial state', () => {
    ...
    it('SS dataLinks contains L4_RAW (mod link) and SLS_L3_IN (SLS) — no SS1 segments', () => {
      if (result.kind !== RESULT_KIND.Ok) return;
      const ss = result.data.subsystems.find(s => Number(s.systemId) === SS)!;
      expect(collectDataLinkIds(ss.children)).toEqual([
        SLS_L3_IN.systemId,
        L4_RAW.systemId,
      ]);
      expect(collectDataLinkIds(ss.children)).not.toContain(
        SLS_L5_OUT.systemId,
      );
      expect(collectDataLinkIds(ss.children)).not.toContain(SLS_L6_IN.systemId);
    });
