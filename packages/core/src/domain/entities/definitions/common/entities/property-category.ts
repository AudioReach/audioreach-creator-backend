...
export abstract class PropertyCategory {
  ...
  AddPropertyDefinition(propertyDefinition: PropertyDefinition): void {
    ...
    const valueWithSameSystemId = this.properties.some(
      v => v.systemId === propertyDefinition.systemId,
    );
    if (valueWithSameSystemId) {
      throw new DuplicateSystemIdException(
        `SystemId ${propertyDefinition.systemId} already exists in PropertyDefinition for Category: ${this.categoryId}`,
      );
    }

    const valueWithSamePropertyId = this.properties.some(
      v => v.propertyId === propertyDefinition.propertyId,
    );
    if (valueWithSamePropertyId) {
      throw new DuplicatePropertyIdException(
        `PropertyId ${propertyDefinition.propertyId} already exists in PropertyDefinition for Category: ${this.categoryId}`,
      );
    }
