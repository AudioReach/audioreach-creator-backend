...
export class SubgraphDriverProperty {
    ...
    AddPropertyDefinition(propertyDefinition: PropertyDefinition): void {
        ...
        const valueWithSameSystemId = this.propertyDefinitions.some(v => v.systemId === propertyDefinition.systemId);
        if (valueWithSameSystemId) {
            throw new DuplicateSystemIdException(`SystemId ${propertyDefinition.systemId} already exists in Subgraph PropertyDefinition`)
        }

        const valueWithSamePropertyId = this.propertyDefinitions.some(v => v.propertyId === propertyDefinition.propertyId);
        if (valueWithSamePropertyId) {
            throw new DuplicatePropertyIdException(`PropertyId ${propertyDefinition.propertyId} already exists in Subgraph PropertyDefinition`)
        }
