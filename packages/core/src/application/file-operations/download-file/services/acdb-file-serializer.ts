...
export class AcdbFileSerializer {
  ...
  private mergeVcpmIntoVoice(
    ...
    const merged = [...voice];
    const bySubgraphId = new Map(merged.map(v => [v.naturalId, v]));

    for (const vcpmEntry of vcpm) {
      const existing = bySubgraphId.get(vcpmEntry.naturalId);
      if (existing) {
        ...
        const mkMap = new Map(
          existing.masterKeys.map(mk => [mk.keyNaturalId, mk.isDynamic]),
        );
        for (const mk of vcpmEntry.masterKeys) {
          if (!mkMap.has(mk.keyNaturalId))
            mkMap.set(mk.keyNaturalId, mk.isDynamic);
        }
        ...
        bySubgraphId.set(vcpmEntry.naturalId, vcpmEntry);
      }
    }

    merged.sort((a, b) => a.naturalId - b.naturalId);
