...
/**
 * Issue phase during entity processing
 */
export const ISSUE_PHASE = {
  PARSING: 'parsing',
  BUILDING: 'building',
  INSERTION: 'insertion',
} as const;

export type IssuePhase = (typeof ISSUE_PHASE)[keyof typeof ISSUE_PHASE];
...
export class IssueCollector {
  ...
  formatForApi(): {errors: string[]; warnings: string[]} {
    return {
      errors: this.getErrors().map(
        err =>
          `[${err.code}] ${err.entityType} (${err.entityIdentifier}): ${err.message}`,
      ),
      warnings: this.getWarnings().map(
        warn =>
          `[${warn.code}] ${warn.entityType} (${warn.entityIdentifier}): ${warn.message}`,
      ),
    };
  }

  /**
   * Get only insertion phase issues
   */
  getInsertionIssues(): EntityBuildIssue[] {
    return this.issues.filter(i => i.phase === ISSUE_PHASE.INSERTION);
  }
}
